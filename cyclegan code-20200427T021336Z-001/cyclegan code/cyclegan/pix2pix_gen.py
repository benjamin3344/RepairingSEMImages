...
# April 19, 2020
#
# Modified version of Pix2Pix U-net generator at
...
def random_jitter(input_image, real_image):
  """Random jittering.
  Resizes to 192 x 192 and then randomly crops to IMG_HEIGHT x IMG_WIDTH.
  Args:
  ...
  """
  # resizing to 192 x 192 x 3
  input_image, real_image = resize(input_image, real_image, 192, 192)

  # randomly cropping to 192 x 192 x 3
...
def unet_generator(output_channels, norm_type='batchnorm'):
  ...
  down_stack = [
      downsample(64, 4, norm_type, apply_norm=False),  # (bs, 96, 96, 64)
      downsample(128, 4, norm_type),  # (bs, 48, 48, 128)
      downsample(256, 4, norm_type),  # (bs, 24, 24, 256)
  ...
  up_stack = [
      ...
      upsample(512, 4, norm_type),  # (bs, 12, 12, 1024)
      upsample(256, 4, norm_type),  # (bs, 24, 24, 512)
      upsample(128, 4, norm_type),  # (bs, 48, 48, 256)
      upsample(64, 4, norm_type),  # (bs, 96, 96, 128)
  ]
...
def discriminator(norm_type='batchnorm', target=True):
  ...
  if target:
    ...
    x = tf.keras.layers.concatenate([inp, tar])  # (bs, 192, 192, channels*2)

  down1 = downsample(64, 4, norm_type, False)(x)  # (bs, 96, 96, 64)
  down2 = downsample(128, 4, norm_type)(down1)  # (bs, 48, 48, 128)
  down3 = downsample(256, 4, norm_type)(down2)  # (bs, 24, 24, 256)

  zero_pad1 = tf.keras.layers.ZeroPadding2D()(down3)  # (bs, 26, 26, 256)
  conv = tf.keras.layers.Conv2D(
      512, 4, strides=1, kernel_initializer=initializer,
      use_bias=False)(zero_pad1)  # (bs, 23, 23, 512)
  ...
  zero_pad2 = tf.keras.layers.ZeroPadding2D()(leaky_relu)  # (bs, 25, 25, 512)

  last = tf.keras.layers.Conv2D(
      1, 4, strides=1,
      kernel_initializer=initializer)(zero_pad2)  # (bs, 22, 22, 1)
