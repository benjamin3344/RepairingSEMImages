...
def train(
    ...
    with tf.name_scope('Inputs'), tf.device("/cpu:0"):
        ...
        # noisy_crop  noisy_edge    clean crop
    ...
    for gpu in range(submit_config.num_gpus):
        with tf.device("/gpu:%d" % gpu):
            ...
            # contrast_high = tf.square((denoised - clean_target_split[gpu]) * 255) + 1
            # contrast_high = tf.square((denoised - clean_target_split[gpu])) + 1
            # contrast_high = (denoised - clean_target_split[gpu]) * 5 + 1
            contrast_high = tf.abs(denoised - clean_target_split[gpu]) * 255 + 1
            contrast_low = tf.ones_like(denoised)
            contrast_weight = tf.where((denoised > 0.0),  contrast_high,  contrast_low)


            if noise2noise:
                # meansq_error = tf.reduce_mean(tf.multiply(tf.square(noisy_input_split[gpu] - denoised),
                                                          #1.0 + noisy_target_split[gpu]))
                meansq_error = tf.reduce_mean(tf.multiply(tf.square(noisy_input_split[gpu] - denoised),
                                                          contrast_weight))
                # meansq_error = tf.reduce_mean(tf.square(noisy_target_split[gpu] - denoised))
            else:
                # meansq_error = tf.reduce_mean(tf.multiply(tf.square(clean_target_split[gpu] - denoised),
                                                          #1.0 + noisy_target_split[gpu]))
                # meansq_error = tf.reduce_mean(tf.multiply(tf.multiply(tf.square(clean_target_split[gpu] - denoised),
                #                                                       contrast_weight),
                #                                           1.0 + noisy_target_split[gpu] / 128.0))
                # meansq_error = tf.reduce_mean(tf.abs(clean_target_split[gpu] - denoised))
                meansq_error = tf.reduce_mean(tf.multiply(tf.multiply(tf.abs(clean_target_split[gpu] - denoised),
                                                                      contrast_weight),
                                                          1.0 + noisy_target_split[gpu]))
