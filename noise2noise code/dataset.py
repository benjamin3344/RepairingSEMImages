...
def random_crop_noised_clean2(x, add_noise):
    cropped = tf.random_crop(resize_small_image(x), size=[3, 256, 256]) / 255.0 - 0.5
    cropped1 = cropped[2,:,:]
    noisy_crop = tf.stack([cropped1]*3)
    cropped2 = cropped[1,:,:]
    clean_crop = tf.stack([cropped2]*3)
    cropped3 = (cropped[0,:,:] + 0.5) * 128.0
    noisy_edge = tf.stack([cropped3]*3)
    return (noisy_crop, noisy_edge, clean_crop)
...
def create_dataset(train_tfrecords, minibatch_size, add_noise):
    ...
    dset = dset.map(lambda x: random_crop_noised_clean2(x, add_noise))
